'''这个脚本为 FastAPI 应用提供了一个用于与 OpenAI API 交互的标准化数据结构，包括输入参数和输出响应的模型。这些模型可以用于请求验证、响应构建以及序列化/反序列化 JSON 数据。'''
...
'''定义和验证聊天补全请求的数据结构, 下列每个属性都对应于 OpenAI API 中的一个参数'''
class OpenAIChatInput(OpenAIBaseInput):
    messages: List[ChatCompletionMessageParam] # 必填项，表示对话消息的历史记录或当前输入的消息列表
    model: str = get_default_llm() # 可选项，默认值由 get_default_llm() 函数提供，指定使用的语言模型
