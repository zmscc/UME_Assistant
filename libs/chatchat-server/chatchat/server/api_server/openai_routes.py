'''这个脚本提供了一个接口，允许用户通过 FastAPI 应用与 OpenAI 兼容的平台进行交互，包括模型管理、聊天补全、文本嵌入、图像生成和编辑以及文件操作等功能。'''
...
'''这个函数是为了发送请求到 OpenAI 的 API，并根据配置添加额外的字段到响应中。它支持流式响应，允许在接收到数据时即时处理和发送数据块。'''
async def openai_request(
    method, # 一个异步函数，代表要调用的 OpenAI API 方法
    body,
    extra_json: Dict = {}, # 要在响应中添加的额外字段
    header: Iterable = [], # header 和 tail：它们是可迭代的对象，分别表示要在响应开始和结束时添加的内容。
    tail: Iterable = []
):
    """
    ...
    '''这个内部函数是一个异步生成器，用于产生 OpenAI API 响应的各个部分。'''
    async def generator():
        try:
            for x in header:
                if isinstance(x, str):
    ...
    params = body.model_dump(exclude_unset=True) # 获取请求参数，并确保未设置的参数不会被包含。
    if params.get("max_tokens") == 0:
        params["max_tokens"] = Settings.model_settings.MAX_TOKENS

    if hasattr(body, "stream") and body.stream: # 如果 body 有 stream 属性且为 True，则返回一个 EventSourceResponse，该响应使用 generator 函数作为数据源。
        return EventSourceResponse(generator())
    else:
        result = await method(**params) # 如果没有启用流式传输，则直接调用 method 并处理结果，添加 extra_json 中的额外字段，然后返回处理后的结果。
        for k, v in extra_json.items():
            setattr(result, k, v)
