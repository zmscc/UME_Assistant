...
class MilvusKBService(KBService): # MilvusKBService 类继承自 KBService
    milvus: Milvus # milvus 是一个类属性，预期是一个 Milvus 类的实例

    @staticmethod
    def get_collection(milvus_name): # 静态方法，接收一个 Milvus 集合的名称，并返回一个 Collection 对象。
        from pymilvus import Collection

        return Collection(milvus_name)

    def get_doc_by_ids(self, ids: List[str]) -> List[Document]: # 它接收一个文档 ID 列表，并返回对应的 Document 对象列表。
        result = []
        if self.milvus.col:
    ...
    def del_doc_by_ids(self, ids: List[str]) -> bool: # 根据提供的 ID 列表删除文档。
        self.milvus.col.delete(expr=f"pk in {ids}")

    @staticmethod
    def search(milvus_name, content, limit=3): # 在 Milvus 集合中搜索内容，并返回最多 limit 个结果。
        search_params = {
            "metric_type": "L2",
            "params": {"nprobe": 10},
        }
    ...
    def vs_type(self) -> str: # 返回支持的向量存储类型，这里是 “MILVUS”。
        return SupportedVSType.MILVUS

    def _load_milvus(self): # 初始化 milvus 实例。
        self.milvus = Milvus(
    ...
    def do_init(self): # 初始化服务。
        self._load_milvus()

    def do_drop_kb(self): # 删除知识库
        if self.milvus.col:
            self.milvus.col.release()
            self.milvus.col.drop()

    def do_search(self, query: str, top_k: int, score_threshold: float): # 在知识库中搜索查询，并返回相关文档。
        self._load_milvus()
    ...
    def do_add_doc(self, docs: List[Document], **kwargs) -> List[Dict]: # 向知识库中添加文档。
        for doc in docs:
    ...
    def do_delete_doc(self, kb_file: KnowledgeFile, **kwargs): # 删除知识库中的文档
