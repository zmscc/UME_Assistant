...
'''
regist_tool 被用作装饰器。当调用 @regist_tool(...) 时，它实际上是在调用 wrapper 函数，并将 greet 函数作为参数传递给它。以下是发生的事情：
1. regist_tool 被调用，但由于没有提供额外的参数，它返回 wrapper 函数。
2. wrapper 函数被 @ 符号应用于 text2sql 函数。
3. wrapper 创建一个 BaseTool 实例，将 text2sql 函数传递给它，并配置标题。
4. 结果是一个 BaseTool 实例，它被添加到 _TOOLS_REGISTRY 中。
'''
@regist_tool(title="数据库对话") # 使用 @regist_tool 装饰器用于注册工具
def text2sql(
    query: str = Field(
    ...
    return BaseToolOutput(query_database(query=query, config=tool_config))# 只传了data，没有传format
